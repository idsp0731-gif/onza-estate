...
import { getAreaArticleBySlug, getPageBlocks } from '@/lib/notion';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = await getAreaArticleBySlug(slug);
  if (!article) return {};
  const areaName = AREA_NAMES[area] ?? area;
  const title = `${article.title}｜${areaName}｜ONZA Estate`;
  const url = `https://www.onza-estate.com/area/${area}/${slug}`;
  return {
    title,
...
export default async function AreaArticlePage({ params }: Props) {
  const { area, slug } = await params;
