...
import { getAkiyaArticleBySlug, getPageBlocks } from '@/lib/notion';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = await getAkiyaArticleBySlug(slug);
  if (!article) return {};
  const description = article.metadescription || undefined;
  const url = `${SITE_URL}/akiya-articles/${slug}`;
  return {
...
export default async function AkiyaArticlePage({ params }: Props) {
  const { slug } = await params;
  const article = await getAkiyaArticleBySlug(slug);
  if (!article) notFound();
