...
import { getInvestmentArticles } from '@/lib/notion';

export const metadata: Metadata = {
  title: '不動産投資コラム一覧｜ONZA Estate',
  description: '不動産投資の考え方、物件選び、資産形成について実務視点で発信しています。',
  alternates: { canonical: 'https://www.onza-estate.com/investment-articles' },
};

export const revalidate = 60;

export default async function InvestmentArticlesPage() {
  const articles = await getInvestmentArticles();
          ...
          {articles.length === 0 ? (
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {articles.map((article) => (
                <Link
                  key={article.id}
                  href={`/investment-articles/${article.slug}`}
