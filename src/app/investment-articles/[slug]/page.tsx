...
import { getInvestmentArticleBySlug, getPageBlocks } from '@/lib/notion';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = await getInvestmentArticleBySlug(slug);
  if (!article) return {};
  const description = article.metadescription || undefined;
  const url = `${SITE_URL}/investment-articles/${slug}`;
  return {
...
export default async function InvestmentArticlePage({ params }: Props) {
  const { slug } = await params;
  const article = await getInvestmentArticleBySlug(slug);
  if (!article) notFound();
    ...
    <div className="min-h-screen bg-[#F5F7F6]">
      ...
      <article className="max-w-3xl mx-auto px-4 py-10 md:py-14">
        <div className="mb-8">
          ...
          {article.date && (
            <p className="text-xs text-[#9CA3AF] font-light">{article.date}</p>
          )}
        </div>
